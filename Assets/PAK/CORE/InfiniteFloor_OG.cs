...
public class InfiniteFloor_OG : MonoBehaviour
{
        ...
        for (int x = -maxGridSize; x <= maxGridSize; x++)
        {
            for (int z = -maxGridSize; z <= maxGridSize; z++)
            {
                ...
                if (distance <= radius / tileSize)
                {
                    ...
                    {
                        // Get the default tile prefab
                        GameObject tile = Instantiate(defaultTilePrefab, new Vector3(tilePosition.x * tileSize, 0, tilePosition.y * tileSize), Quaternion.identity);
