...
public class FollowMouse : MonoBehaviour
{
    ...
    public ScreenWriter debugScreenWriter;
    ...
    void Update()
    {
        ...
        if (Cursor.lockState != CursorLockMode.Locked)
        {
            ...
            // remap coordinates if enabled
            remappedCoords = remapCoords ? RemapCoordinates(normalizedCoords, min, max) : normalizedCoords;

            // Debug output (remove in production)
            debugScreenWriter.UpdateMessage($"Screen Position: X{remappedCoords.x:F0}, Y{remappedCoords.y:F0}");
        }
        else
        {
